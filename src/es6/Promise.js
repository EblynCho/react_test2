...
// 자바스크립트는 기본적으로 1 thread 비동기 처리 방식을 사용하고 있음
// thread : 프로그램이 실행되는 최소 단위
// cf. java : multi thread 가능
// 자바스크립트에서 순차적으로 실행을 하고 싶을 경우 콜백 함수를 이용하여야 함
// 순차적으로 실행할 것이 많아지게 되면 콜백지옥이라고 불리는 형태가 만들어지게 됨
// Promise 는 이러한 콜백 지옥을 해결하기 위해서 사용하는 객체임
...
// Promise 실행 시 콜백함수가 실행되고, 해당 콜백 함수의 매개변수로 resolve(), reject() 이라는 함수를 제공함
...
// Promise 에는 완료 및 오류 처리를 위해서 then(), catch() 함수를 제공하고 있음
...
function getData() {
    return new Promise(function (resolve, reject) {
        const data = 100;
        resolve(data);  // -> then() 동작
        // reject(data);  // -> catch() 동작
    });
}
...
getData()
    ...
    .catch(function (err) {
        console.log(`프로미스 사용 오류시 출력 : ${err}`);
    });
